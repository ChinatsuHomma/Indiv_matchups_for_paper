    ...
    if(!focal_tmp$edge == 1){
        ...
        if(opponentN_tmp == 0){
            ...
            result_tmp <- data.frame(
                ...
                "Focal_DBH"   = rep(focal_tmp$dbh2017,  opponentN_tmp),
                "Opponent_ID" = opponent_tmp$ID,
                "Opponent_Sp" = opponent_tmp$Sp,
                "Opponent_DBH"= opponent_tmp$dbh2017,
...
if(saveFiles){
  write.csv(matchupList, "output/02matchupList/04MatchupListPoles.csv", row.names = FALSE)
}
...
if(saveFiles){
  write.csv(matchupTable, "output/03matchupTable/04MatchupTablePoles.csv", row.names = TRUE)
}
