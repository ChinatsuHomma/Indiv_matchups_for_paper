...
#### Read data ####
subject2x2 <- read.csv("output/01subjects/02SubSmallSaplings2x2.csv")
subjectFP  <- read.csv("output/01subjects/02SubSmallSaplingsFP.csv")
...
if(saveFiles){
  write.csv(matchupList, "output/02matchupList/02MatchupListSmallSaplings.csv", row.names = FALSE)
}
...
if(saveFiles){
  write.csv(matchupTable, "output/03matchupTable/02MatchupTableSmallSaplings.csv", row.names = TRUE)
}
