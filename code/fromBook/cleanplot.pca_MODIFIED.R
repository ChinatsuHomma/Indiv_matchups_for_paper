...
#        if (is.matrix(mat)) {
    ...
    plot(
      mat[, c(ax1, ax2)],
      type = "n",
      #main = paste("PCA biplot - Scaling", scaling), # MODIFIED!
      xlim = ifelse(xlim == c(0,0), c(lim[1, ax1], lim[2, ax1]), xlim),
      ylim = ifelse(ylim == c(0,0), c(lim[1, ax2], lim[2, ax2]), ylim),
      xlab = "", # MODIFIED!
      ylab = "", # MODIFIED!
      asp = 1,
      cex.axis = 0.6,  # Modified!
      #cex.lab = cex.lab,  # Modified!
      las = 2 # Modified!
    )
    abline(h = 0, v = 0, col = "grey60")
    
    # Draw the species scores # Modified!
    par(xpd = TRUE) # Modified!
    if (plot.spe) {
      arrows(
        0,
        0,
    ...
    par(xpd = FALSE) # Modified!

    # Draw the site scores # modified!
    if (plot.sites) {
